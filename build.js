...
const OUTPUT_MAP = {
  win32: "./dist/hymachi-windows.exe",
};
...
if (!pkgConfig)
  throw new Error(
    `Invalid target, must supply ${targetKey} in the package.json`
  );
...
require("child_process").execSync(
  `npm i --target_arch=x64 --target_platform=${target}`,
...
if (!outputName)
  throw new Error("Must specify appropriate output name in config");

console.log("Compiling, output to", outputName);

require("child_process").execSync(
  `pkg ./ --output ${outputName} --compress Brotli`,
  { stdio: ["pipe", "pipe", "pipe"] }
);
